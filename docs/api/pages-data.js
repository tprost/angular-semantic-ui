 ...
 */
module.exports = function generatePagesDataProcessor(log, moduleMap) {

  var navGroupMappers = {
    api: function(areaPages, area) {
      // console.log(_(areaPages).filter('module').groupBy('module').value());
      var navGroups = _(areaPages)
            ...
            .map(function(modulePages, moduleName) {
              ...
              return {
                name: moduleName,
                href: modulePage ? modulePage.path : null,
                type: 'group',
                navItems: navItems
              };
            })

            .filter(function(navGroup) {
              return navGroup.navItems.length > 0;
            })
  ...
  return {
    ...
    $process: function(docs) {
      ...
      docs.push({
        docType: 'nav-data',
        id: 'nav-data',
        template: 'nav-data.template.json',
        outputPath: 'js/nav-data.json',
        areas: areas
      });
      ...
      docs.push({
        docType: 'pages-data',
        id: 'pages-data',
        template: 'pages-data.template.json',
